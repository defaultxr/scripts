...
Some of the scripts in this collection I find most useful are [`a2v`](#a2v), [`ask-yes-no`](#ask-yes-no), [`c`](#c), [`capture-audio`](#capture-audio), [`check-df`](#check-df), [`check-memory-usage`](#check-memory-usage), [`convert-audio`](#convert-audio), [`define-selected`](#define-selected), [`dir-diff`](#dir-diff), [`filter`](#filter), the [`findtype`](#findtype) suite, [`git-multi-log`](#git-multi-log), [`git-pull-subdirs`](#git-pull-subdirs), [`input-passthru`](#input-passthru), [`loc`](#loc), [`lyrics`](#lyrics), [`lyrics-gui`](#lyrics-gui), [`move-gui`](#move-gui), [`myip`](#myip), [`p`](#p), [`pgr`](#pgr), [`recently`](#recently), [`rename-gui`](#rename-gui), [`run-if-not`](#run-if-not), [`set-wallpaper`](#set-wallpaper), [`speak`](#speak), [`sshot`](#sshot), [`ssuspend`](#ssuspend), [`synonyms`](#synonyms), [`tag-gui`](#tag-gui), [`text-gui`](#text-gui), [`translate-selected`](#translate-selected), [`trash-confirm`](#trash-confirm), [`v`](#v), [`vol`](#vol), and [`xeph`](#xeph).
...
List files that only exist in one of the two specified dirs.
